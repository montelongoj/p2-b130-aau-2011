...
The Rotate method in the Shape3D object is used to rotate the object. The method takes one input parameter, which is a RotateMatrix called mat. Next the method creates a new Polygon array newP, and runs through all Polygon objects in the Shape3D. The method then calls the Rotate method within the Polygon object on the current polygon, and inserts the results into the previously created Polygon array newP. The method also inserts the current Polygons Pen into the newP Polygons Pen. The function can be seen on Listing \kode{Shape3DRotate} function in Shape3D}{Shape3DRotate}{Shape3DRotate}.

The ScaleSize method in the Shape3D object is almost the same as the Rotate method in ScaleSize. The only difference is that instead of calling the Rotate method in the Polygon object, the ScaleSize method calls the ScaleSize in the Polygon object.  

The RePaint method in the Shape3D object is used to mark one item in the suitcase - one polygon, with a thicker line than others. In the program this is used when the user clicks on an item in the list of items to the right of the drawing of the suitcase. If the user clicks on an item in the list, the program will mark that item with thicker lines. Another time when this function is used, is when the 3D viewer is opened - a small cube with dimensions 0.5 x 0.5 x 0.5 is drawn in the suitcases 0,0,0 point, so the user knows where the 0,0,0 point is. This cube is also marked with a thicker width than the items.

The method takes two integers as input parameters: num and col\_w. The num input is the item that has been selected, and the col\_w is how with the line must be. The method initiates a new Polygon array, newP, where all the polygons will be stored. The method also contains three counters. One that counts the counts the elements in the polygon, this counter is called "element\_counter". Here an element consists of four sides. Another counter counts is counted up when a polygon is made thicker, this is counter is called "counter". The last counter is counted up regardless if a polygon is made thicker or not. This is the counter called "count" which keeps track of the Polygon array newP.
