...
When all the points are packed, it will call the method to generate the colors for the items \fxfatal{lav ref}. The items must have a color, but no color must be the same as the item next to it, so they will not be mixed for the user. If some items could not be packed, they will be shown to the user, and the form to show the suitcase in 3D will appear \fxfatal{lav ref}.
